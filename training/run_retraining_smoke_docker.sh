...
RUN_ID="${RETRAIN_RUN_ID:-smoke-$(date -u +%Y%m%dT%H%M%SZ)}"
...
FORCE_QUALITY_GATE_PASS="${FORCE_QUALITY_GATE_PASS:-1}"

mkdir -p "${OUT_DIR}"
RENDERED_CONFIG="${OUT_DIR}/${RUN_ID}_config.yaml"
RESULT_JSON="${OUT_DIR}/${RUN_ID}_result.json"

echo "[1/3] Rendering run-specific config for ${RUN_ID}"
EXTRA_RENDER_ARGS=()
...
echo "[2/3] Running retraining container"
docker run --rm \
  --gpus all \
  --shm-size=10g \
...
echo "[3/3] Verifying result JSON + MLflow + registry"
docker run --rm \
  -v "${OUT_DIR}:/out" \
  -v "${REPO_ROOT}:/work" \
  -e PYTHONPATH=/work/training \
...
echo "Smoke retraining test passed for ${RUN_ID}"
