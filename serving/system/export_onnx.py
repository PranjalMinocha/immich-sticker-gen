...
Run once before starting serving containers.
Reuses the same export logic as benchmark.py.
"""
import os, warnings
...
def export_encoder(sam, path: Path):
    if path.exists():
        print(f"Encoder already exported: {path}")
        return
    ...
    with warnings.catch_warnings():
        ...
        torch.onnx.export(
            sam.image_encoder, dummy, str(path),
            input_names=["image"],
...
def export_decoder(sam, path: Path):
    if path.exists():
        print(f"Decoder already exported: {path}")
        return
